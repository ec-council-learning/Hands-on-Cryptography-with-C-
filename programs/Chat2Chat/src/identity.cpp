...
c2c::Identity::Identity(const std::string& id_file)
  ...
  ))
{
  ...
  // or creates a new identity and exports to file data/id_ecdh.c2cid
  else {
    GenerateRandom();
    Export(id_file);
  }
...
c2c::Identity::Identity(const Identity& rht)
...
std::string c2c::Identity::Fingerprint() const
{
...
void c2c::Identity::GenerateRandom()
{
...
const c2c::Identity& c2c::Identity::Import(const std::string& filename)
{
...
void c2c::Identity::Export(const std::string& filename) const
{
