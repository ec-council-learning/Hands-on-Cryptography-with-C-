...
wiz::Identity::Identity(const std::string& id_file)
  ...
  ))
{
  ...
  // or creates a new identity and exports to file data/id_ecdh.wizid
  else {
    GenerateRandom();
    Export(id_file);
  }
...
wiz::Identity::Identity(const Identity& rht)
...
std::string wiz::Identity::Fingerprint() const
{
...
wiz::Identity::operator std::string() const {
  auto pub_bytes  = Botan::X509::PEM_encode(*m_public_key); // expects pubkey

  std::stringstream stream;
  stream << pub_bytes << std::flush;

  return stream.str();
}

void wiz::Identity::GenerateRandom()
{
...
const wiz::Identity& wiz::Identity::Import(const std::string& filename)
{
...
void wiz::Identity::Export(const std::string& filename) const
{
